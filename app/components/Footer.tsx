...
const cities = [
  ...
  { name: "Gujranwala", count: 18 },
  { name: "Peshawar", count: 14 },
...
const propertyTypes = [
  ...
  { label: "Offices", href: "/listings?type=office" },
  { label: "Shops", href: "/listings?type=shop" },
];
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div className="lg:col-span-1">
            ...
            <p className="text-white/60 text-sm leading-relaxed mb-5">
              Pakistan ka sabse bharosemand rental platform. Apna ghar dhundein asani se.
            </p>
            {/* Social Icons */}
            <div className="flex items-center gap-3">
              {/* Facebook */}
              <a href="#" aria-label="Facebook"
                className="w-8 h-8 rounded-full border border-white/20 flex items-center justify-center hover:border-[#f0c040] hover:text-[#f0c040] text-white/60 transition-colors duration-200">
          ...
          {/* Quick Links */}
          <div>
            <h3 className="text-[#f0c040] font-semibold text-sm uppercase tracking-widest mb-4">
              Quick Links
            </h3>
            <ul className="space-y-2.5">
              {quickLinks.map((link) => (
                <li key={link.label}>
                  <Link href={link.href}
                    className="text-white/60 text-sm hover:text-[#f0c040] transition-colors duration-200 flex items-center gap-2 group">
              ...
              {cities.map((city) => (
                <li key={city.name}>
                  <Link
                    ...
                    className="text-white/60 text-sm hover:text-[#f0c040] transition-colors duration-200 flex items-center justify-between group"
                  >
                    <span className="flex items-center gap-2">
                      <span className="w-1 h-1 rounded-full bg-[#f0c040]/40 group-hover:bg-[#f0c040] transition-colors" />
                      {city.name}
                    </span>
                    <span className="text-xs text-white/30 group-hover:text-[#f0c040]/60 transition-colors">
                      {city.count}
                    </span>
                  </Link>
                </li>
            ...
            <ul className="space-y-4">
              <li>
                <a href="tel:+923111445559"
                  className="flex items-start gap-3 text-white/60 text-sm hover:text-[#f0c040] transition-colors duration-200 group">
                  <svg className="w-4 h-4 mt-0.5 shrink-0 group-hover:text-[#f0c040]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="border-t border-white/10">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-2">
          <p className="text-white/40 text-xs">
            © {new Date().getFullYear()} RentGhar. Sab haqooq mahfooz hain.
          </p>
